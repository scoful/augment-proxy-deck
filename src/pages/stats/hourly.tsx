...
export default function HourlyStats() {
  ...
  const { data: hourlyStats, isLoading, error, isFetching } = api.stats.getHourlyStats.useQuery(
    undefined,
    {
      // 启用轮询，每60秒更新一次数据
  ...
  const getActivityLevel = (count: number, maxCount: number) => {
    ...
    if (ratio >= 0.4) return { level: "中", color: "bg-yellow-100 text-yellow-800" };
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-slate-800">{formatNumber(hourlyStats.summary.yesterdayTotal)}</p>
                    </div>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
                  ...
                  <div className="mt-4">
                    <span className="text-xs text-slate-500">
                      vs 昨日: {hourlyStats.summary.todayUsers > hourlyStats.summary.yesterdayUsers ? '+' : ''}{hourlyStats.summary.todayUsers - hourlyStats.summary.yesterdayUsers}人
                    </span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-200">
                    <h3 className="text-lg font-semibold text-slate-800">今日24小时数据</h3>
                  </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="bg-white divide-y divide-slate-200">
                        {hourlyStats.today.map((data, index) => {
                          const maxCount = Math.max(...hourlyStats.today.map(d => d.count));
                          const activity = getActivityLevel(data.count, maxCount);
                          return (
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-200">
                    <h3 className="text-lg font-semibold text-slate-800">昨日24小时数据</h3>
                  </div>
                  <div className="overflow-x-auto max-h-96 overflow-y-auto">
                    <table className="w-full">
                      <thead className="bg-slate-50 sticky top-0">
                      ...
                      <tbody className="bg-white divide-y divide-slate-200">
                        {hourlyStats.yesterday.map((data, index) => {
                          const maxCount = Math.max(...hourlyStats.yesterday.map(d => d.count));
                          const activity = getActivityLevel(data.count, maxCount);
                          return (
                            <tr key={index} className="hover:bg-slate-50">
                              <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-slate-900">
