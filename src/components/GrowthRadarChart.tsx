...
export default function GrowthRadarChart({ todayData, yesterdayData }: GrowthRadarChartProps) {
  // 计算增长率数据，按时段分组
  ...
  const stats = {
    ...
    avgGrowth: (chartData.reduce((sum, d) => sum + d.增长率, 0) / chartData.length).toFixed(1),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-full">
        ...
        <div className="flex flex-col justify-center space-y-4">
          <div className="bg-slate-50 rounded-lg p-4">
            <h4 className="text-sm font-medium text-slate-700 mb-3">增长率统计</h4>
