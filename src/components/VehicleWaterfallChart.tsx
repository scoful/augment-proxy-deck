...
export default function VehicleWaterfallChart({
    ...
    if (active && payload?.length) {
      ...
      return (
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-lg">
          <p className="font-medium text-slate-800 mb-2">{`日期: ${label}`}</p>
  ...
  const stats = chartData.length > 0 ? {
    totalNewActive: chartData.reduce((sum, d) => sum + d.rawData.newActive, 0),
    totalRemovedInactive: chartData.reduce((sum, d) => sum + d.rawData.removedInactive, 0),
    totalNewlyInactive: chartData.reduce((sum, d) => sum + d.rawData.newlyInactive, 0),
    totalSameDayInactive: chartData.reduce((sum, d) => sum + d.rawData.sameDayInactive, 0),
    avgDailyChange: chartData.reduce((sum, d) => sum + d.rawData.totalVehicleChange, 0) / chartData.length,
  } : null;

  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-slate-800">车辆变化场景分析</h3>
        <p className="text-sm text-slate-600">
          分组展示车辆变化的4种场景：正向变化（新增可用、移除不可用）vs 负向变化（新增不可用、当天新增当天不可用），紫色折线显示净变化趋势
        </p>
      </div>
