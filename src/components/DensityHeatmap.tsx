...
export default function DensityHeatmap({ todayData, yesterdayData }: DensityHeatmapProps) {
  // 获取当前时间
  const currentHour = new Date().getHours();
  
  // 计算最大值用于归一化
  const allValues = [...todayData.map(d => d.count), ...yesterdayData.map(d => d.count)];
      ...
      <div className="flex flex-col">
        ...
        <div className="mb-4 grid grid-cols-3 gap-4 text-xs text-slate-600">
          <div className="text-center">
            ...
            <div>{Math.max(...todayData.map(d => d.count)).toLocaleString()}</div>
          </div>
          <div className="text-center">
            <div className="font-medium text-slate-800">昨日峰值</div>
            <div>{Math.max(...yesterdayData.map(d => d.count)).toLocaleString()}</div>
          </div>
          <div className="text-center">
            <div className="font-medium text-slate-800">当前时间</div>
            <div className="text-blue-600">{currentHour.toString().padStart(2, '0')}:00</div>
          </div>
        </div>
              ...
              <div className="flex-1 grid grid-cols-24 gap-0.5">
                {dayData.hours.map((hourData, hourIndex) => (
                  <div
                    key={hourIndex}
                    className={`
                      aspect-square rounded-sm border border-slate-200 flex items-center justify-center
                      cursor-pointer transition-all duration-200 hover:scale-110 hover:z-10 relative
                      ${hourData.isFuture ? 'opacity-50 border-dashed' : ''}
                    `}
                    style={{ 
        ...
        <div className="mt-4 flex items-center justify-center gap-4">
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              <div key={`density-${item.level}-${index}`} className="flex items-center gap-1">
                <div
