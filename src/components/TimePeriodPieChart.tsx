...
export default function TimePeriodPieChart({ todayData, yesterdayData }: TimePeriodPieChartProps) {
  // 定义时段分类函数
  ...
  // 分析今日数据
  const todayCategories = todayData.reduce((acc, data) => {
  ...
  // 分析昨日数据
  const yesterdayCategories = yesterdayData.reduce((acc, data) => {
    const category = categorizeByCount(data.count);
    acc[category] = (acc[category] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
  ...
  // 准备昨日饼图数据
  const yesterdayPieData = Object.entries(yesterdayCategories).map(([name, value]) => ({
    name,
    value,
    percentage: ((value / 24) * 100).toFixed(1)
  }));
    ...
    <div className="w-full h-80">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-full">
        ...
        <div className="flex flex-col">
          <h4 className="text-sm font-medium text-slate-700 mb-2 text-center">今日时段分布</h4>
        ...
        {/* 昨日时段分布 */}
        <div className="flex flex-col">
          <h4 className="text-sm font-medium text-slate-700 mb-2 text-center">昨日时段分布</h4>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
