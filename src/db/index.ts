...
export function createDatabase(d1Database?: any) {
  ...
  if (env.isCloudflare && d1Database) {
    ...
    // Vercel + Turso环境 - 使用预初始化的连接
    console.log("🚀 Using Turso database for Vercel deployment");

    // 对于 Vercel 环境，应该使用预初始化的 Turso 连接
    // 这里不能使用动态导入，因为函数需要同步返回
    throw new Error(
      "Turso connection must be pre-initialized for Vercel. Use initializeTursoDatabase() first.",
    );
  } else {
