...
export const userStatsDetail = sqliteTable('user_stats_detail', {
...
export const vehicleStatsDetail = sqliteTable('vehicle_stats_detail', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  ...
  carRecordedIdx: index('idx_vehicle_detail_recorded').on(table.carId, table.recordedAt),
  carTypeIdx: index('idx_vehicle_detail_type').on(table.carType, table.recordedAt),
}));

// 车辆统计汇总表 - 每日00:05采集summary字段
export const vehicleStatsSummary = sqliteTable('vehicle_stats_summary', {
  id: integer('id').primaryKey({ autoIncrement: true }),
...
export const systemStatsDetail = sqliteTable('system_stats_detail', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  ...
  hourDateIdx: index('idx_system_detail_hour_date').on(table.hourTimestamp, table.dataDate),
  dataDateIdx: index('idx_system_detail_date').on(table.dataDate),
}));

// 系统统计汇总表 - 每日00:05采集summary字段
export const systemStatsSummary = sqliteTable('system_stats_summary', {
  id: integer('id').primaryKey({ autoIncrement: true }),
