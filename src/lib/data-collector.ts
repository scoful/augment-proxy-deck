 ...
 * 带重试的API请求
 */
async function fetchWithRetry(url: string, retries = RETRY_CONFIG.maxRetries): Promise<any> {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      console.warn(`API request failed (attempt ${i + 1}/${retries + 1}):`, error);
...
export async function collectDailyStats(d1Database?: D1Database) {
  ...
  try {
    ...
    results.errors.push(`用户数据采集失败: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }

  try {
    results.vehicleSummary = await collectVehicleStatsSummary(d1Database);
  } catch (error) {
    results.errors.push(`车辆汇总数据采集失败: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }

  try {
    results.system = await collectSystemStats(d1Database);
  } catch (error) {
    results.errors.push(`系统数据采集失败: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }

  // 输出采集结果
  const successCount = [results.user, results.vehicleSummary, results.system].filter(Boolean).length;
  const totalTasks = 3;

  if (results.errors.length === 0) {
    console.log(`🎉 每日数据采集完成! 成功: ${successCount}/${totalTasks}`);
  } else {
    console.warn(`⚠️ 每日数据采集部分失败! 成功: ${successCount}/${totalTasks}, 错误: ${results.errors.length}`);
